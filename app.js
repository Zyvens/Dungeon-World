(() => {
  ...
  function sanitizeNotesHtml(raw){
    ...
    nodes.forEach((el)=>{
      ...
      const wasSpoiler=el.tagName==="SPAN" && /(^|\s)spoiler(\s|$)/.test(el.className||"");
      const color=el.tagName==="FONT"?el.getAttribute("color"):null;
      const size=el.tagName==="FONT"?el.getAttribute("size"):null;
      [...el.attributes].forEach((attr)=>el.removeAttribute(attr.name));
      if(wasSpoiler) el.className="spoiler";
  ...
  function normalize(incoming){
    ...
    s.advancedMoves=Array.isArray(incoming.advancedMoves)?incoming.advancedMoves.filter(x=>typeof x==="string"):[];
    s.advancedNotes=String(incoming.advancedNotes||"");
    ...
    s.status.xp=Math.max(0,Math.floor(Number(s.status.xp)||0));
  ...
  function applyTheme(){
    ...
    root.style.setProperty("--primary",t.primary); root.style.setProperty("--accent",t.accent); root.style.setProperty("--panel",t.panel); root.style.setProperty("--text",t.text); root.style.setProperty("--font-scale",t.fontScale); root.style.setProperty("--panel-opacity",t.panelOpacity); root.style.setProperty("--shade",t.shade);
    ...
    $("campaignName").value=t.campaignName; $("primaryColor").value=t.primary; $("accentColor").value=t.accent; $("panelColor").value=t.panel; $("textColor").value=t.text; $("fontScale").value=t.fontScale; $("panelOpacity").value=t.panelOpacity; $("backgroundShade").value=t.shade;
  }

  function renderClassSelectors(){
    const sel=$("classSelect"); sel.innerHTML=Object.entries(CLASSES).map(([id,k])=>`<option value="${id}">${esc(k.name)}</option>`).join(""); sel.value=state.identity.classId;
    const k=klass(); $("heritageLabel").textContent=k.heritageLabel||"Raça"; $("alignmentLabel").textContent=k.alignmentLabel||"Alinhamento";
    const hs=$("heritageSelect"); hs.innerHTML=(k.heritages||[]).map(h=>`<option value="${h.id}">${esc(h.name)}</option>`).join(""); hs.value=state.identity.heritageId; $("heritageLabelWrap").classList.toggle("hidden",!(k.heritages||[]).length);
  ...
  function renderSetup(){
    ...
    wrap.innerHTML="";
    k.setup.forEach(item=>{const box=document.createElement("div");box.className="choice-box";box.innerHTML=`<strong>${esc(item.label)}</strong>`;
      if(item.type==="select"){const s=document.createElement("select");s.innerHTML=item.options.map(o=>`<option>${esc(o)}</option>`).join("");s.value=bag[item.key]||item.options[0];s.addEventListener("change",()=>{bag[item.key]=s.value;schedulePersist();});box.appendChild(s);}
      else if(item.type==="multi"){const m=document.createElement("div");m.className="multi-options";m.innerHTML=item.options.map(o=>`<label><input type="checkbox" value="${esc(o)}" ${(bag[item.key]||[]).includes(o)?"checked":""}/> <span>${esc(o)}</span></label>`).join("");m.addEventListener("change",e=>{const checked=[...m.querySelectorAll("input:checked")];if(item.max&&checked.length>item.max){e.target.checked=false;toast(`Escolha no máximo ${item.max} opções.`,true);return;}bag[item.key]=[...m.querySelectorAll("input:checked")].map(x=>x.value);schedulePersist();});box.appendChild(m);}
      else{const input=document.createElement("input");input.placeholder=item.placeholder||"";input.value=bag[item.key]||"";input.addEventListener("input",()=>{bag[item.key]=input.value;schedulePersist();});box.appendChild(input);}wrap.appendChild(box);
    });
  }

  function renderMoves(){
    const k=klass(),level=Number(state.status.level)||1,split=Number.isInteger(k.advancedSplit)?k.advancedSplit:(k.advanced||[]).length,firstOnly=new Set(k.firstAdvanceOnly||[]);
    $("startingMoves").innerHTML=(k.startingMoves||[]).map(m=>`<div class="move-card"><h3>${esc(m.name)}</h3><p>${esc(m.description)}</p></div>`).join("");
    const adv=$("advancedMoves");
    adv.innerHTML=(k.advanced||[]).map((name,index)=>{
      const tier=index<split?"2–5":"6–10";
      const unlocked=level>=2 && (index<split || level>=6);
      const checked=state.advancedMoves.includes(name);
      const first=firstOnly.has(name)?" · apenas no primeiro avanço":"";
      return `<label class="advanced-option ${unlocked?"":"locked"}" title="Níveis ${tier}${first}"><input type="checkbox" value="${esc(name)}" ${checked?"checked":""} ${unlocked||checked?"":"disabled"}/><span>${esc(name)} <small>níveis ${tier}${first}</small></span></label>`;
    }).join("");
    ...
    $("moveIntro").textContent=level<2?"No nível 1, use apenas os movimentos iniciais. Movimentos avançados são liberados a partir do nível 2.":level<6?"Movimentos avançados de níveis 2–5 estão liberados; os de níveis 6–10 permanecem bloqueados.":"Movimentos de níveis 2–5 e 6–10 estão liberados conforme as regras da classe.";
  }
  ...
  function renderEquipment(){
    ...
    state.equipment.forEach(item=>{const node=$("equipmentTemplate").content.firstElementChild.cloneNode(true);node.dataset.id=item.id;node.querySelector('[data-k="name"]').value=item.name||"";node.querySelector('[data-k="description"]').value=item.description||"";node.querySelector('[data-k="weight"]').value=Number(item.weight)||0;node.querySelector('[data-k="quantity"]').value=Number(item.quantity)||0;
      node.querySelectorAll("[data-k]").forEach(el=>el.addEventListener("input",()=>{const key=el.dataset.k;item[key]=(key==="weight"||key==="quantity")?Math.max(0,Number(el.value)||0):el.value;if(key==="quantity")item[key]=Math.floor(item[key]);syncCalculated();schedulePersist();}));
  ...
  function renderPeople(){
    ...
    state.people.forEach(person=>{const node=$("personTemplate").content.firstElementChild.cloneNode(true);node.dataset.id=person.id;const img=node.querySelector("img");img.src=person.image||PLACEHOLDER;
      ...
      node.querySelector('[data-k="imageInput"]').addEventListener("change",async e=>{const f=e.target.files[0];if(!f)return;try{person.image=await resizeImage(f,700,.82);img.src=person.image;schedulePersist();}catch(err){toast(err.message||"Imagem inválida.",true);}});
  ...
  function bindStatic(){
    ...
    $("level").addEventListener("input",e=>{state.status.level=Math.min(10,Math.max(1,Number(e.target.value)||1));renderIdentityRules();renderMoves();schedulePersist();});
    $("xp").addEventListener("input",e=>{state.status.xp=Math.max(0,Number(e.target.value)||0);schedulePersist();});
    $("hpCurrent").addEventListener("input",e=>{const v=Number(e.target.value);state.status.hpCurrent=Number.isFinite(v)?Math.max(0,v):0;schedulePersist();});
    $("armor").addEventListener("input",e=>{state.status.armor=Math.max(0,Number(e.target.value)||0);schedulePersist();});
    ...
    $("notesEditor").addEventListener("input",e=>{state.notesHtml=sanitizeNotesHtml(e.currentTarget.innerHTML);schedulePersist();});
    $("notesEditor").addEventListener("click",e=>{if(e.target.classList.contains("spoiler"))e.target.classList.toggle("revealed");});
    document.querySelectorAll("[data-cmd]").forEach(b=>b.addEventListener("click",()=>{document.execCommand(b.dataset.cmd,false,b.dataset.value||null);$("notesEditor").focus();state.notesHtml=sanitizeNotesHtml($("notesEditor").innerHTML);schedulePersist();}));
    $("fontSizeSelect").addEventListener("change",e=>{document.execCommand("fontSize",false,e.target.value);state.notesHtml=sanitizeNotesHtml($("notesEditor").innerHTML);schedulePersist();});
    $("fontColor").addEventListener("input",e=>{document.execCommand("foreColor",false,e.target.value);state.notesHtml=sanitizeNotesHtml($("notesEditor").innerHTML);schedulePersist();});
    $("spoilerBtn").addEventListener("click",()=>{const sel=window.getSelection();const text=sel?.toString()||"Conteúdo oculto";document.execCommand("insertHTML",false,`<span class="spoiler">${esc(text)}</span>`);state.notesHtml=sanitizeNotesHtml($("notesEditor").innerHTML);schedulePersist();});
    ...
    $("backgroundInput").addEventListener("change",async e=>{const f=e.target.files[0];if(!f)return;try{state.theme.backgroundImage=await resizeImage(f,1800,.78);applyTheme();schedulePersist();}catch(err){toast(err.message||"Imagem inválida.",true);}});
    $("portraitInput").addEventListener("change",async e=>{const f=e.target.files[0];if(!f)return;try{state.images.portrait=await resizeImage(f,900,.84);$("portraitImg").src=state.images.portrait;schedulePersist();}catch(err){toast(err.message||"Imagem inválida.",true);}});
  ...
  async function init(){
    try{await import("./class-rules.js");}catch(err){console.warn("Metadados de nível não carregados",err);}
    bindStatic();
    const p=cloudParams();sheetId=p.id;editToken=p.key;
    const raw=localStorage.getItem(STORAGE_KEY);if(raw){try{state=normalize(JSON.parse(raw));}catch(e){state=clone(DEFAULT_STATE);}}
